...
class BitwiseData:
    ...
    def __init__(self, count=None, value=0, msb_first=False):
        ...
        if (count is None):
            ...
            raise ValueError(
                "count parameter %d is insufficeint to hold value with hi bit %d" % (count, minBits))
        else:
            self.count = count
        if msb_first:
    ...
    def __getitem__(self, key):
        if isinstance(key, slice):
            ...
            if len(s) < 1:
                raise ValueError(
                    "Cannot slice BinaryData to zero bits (%s)" % key)
    ...
    def convert(origin, count=None):
        ...
        try:
            ...
            raise NotImplementedError(
                f'No conversion exists for type {type(origin)}')

    @staticmethod
    def convertBytes(origin, msb_first=False):
        if type(origin) != bytes:
            raise NotImplementedError(
                f'convertBytes only works on type "bytes", not "{type(origin)}"')
        bd_array = [BitwiseData(8, b, msb_first) for b in origin]
        return BitwiseData.concat(bd_array)

    def toBytes(self, msb_first=False):
        result = []
        count = self.count
        value = self.value
        while count > 0:
            b = BitwiseData(8, value & 0xFF)
            if msb_first:
                result.append(b.reversed().value)
            else:
...
if __name__ == '__main__':
    ...
    class BitwiseDataTests(unittest.TestCase):
        ...
        def test_toBytes(self):
            a = BitwiseData(8, 0b10010110).toBytes(msb_first=False)
            self.assertEqual(a, bytes([0x96]))
            b = BitwiseData(16, 0b0111110010010110).toBytes(msb_first=False)
            self.assertEqual(b, bytes([0x96, 0x7C]))
        ...
        def mock_rng(self, mockBitCount, mockBits):
            def rng(bitCount):
                self.assertEqual(
                    bitCount, mockBitCount, f'This mock only responds to bit counts of {mockBitCount}')
                return mockBits
            return rng
